...
public class ShieldEffect : Effect
{
        ...
        else
        {
            ...
            if (actionnerType == ActionnerType.ENEMY && Actionner != null)
            {
                if (CanBeDisable)
                {
                    ...
                    if (!enemySlotView.ToggleableEffects.Contains(this))
                    {
                        enemySlotView.ToggleableEffects.Add(this);
                    }
                }
                if (targetModeInfo.targetMode == TargetMode.Manual)
                {
            ...
            else if (actionnerType == ActionnerType.PLAYER && Actionner != null)
            {
                if (CanBeDisable)
                {
                    ...
                    if (!permanentView.ToggleableEffects.Contains(this))
                    {
                        permanentView.ToggleableEffects.Add(this);
                    }
                }
                if (targetModeInfo.targetMode == TargetMode.Manual)
                {
    ...
    {
        Disabled = true;
        UnShieldGA unShieldGA = new(TargetForLinked_Player, TargetForLinked_Enemy);
        unShieldGA.Actionner = Actionner;
